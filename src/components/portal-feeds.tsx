...
export function SocialTimeline({ data }: {
    ...
    return <><div className="portal-filters social-profile-filters" aria-label={t("filterProfile")}><button type="button" aria-pressed={username === "all"} className={username === "all" ? "selected" : ""} onClick={() => setUsername("all")}>{t("all")}</button>{socialProfiles.map(profile => <button type="button" key={profile.username} aria-pressed={username === profile.username} className={username === profile.username ? "selected" : ""} onClick={() => setUsername(profile.username)}>{profile.name}</button>)}</div><p className="feed-result-count" role="status">{t("postCount", {count:posts.length})}</p>{posts.length ? <SocialFeed data={{ ...data, items: posts }}/> : <p className="portal-filter-empty">{t("emptyProfile")}</p>}</>;
}
